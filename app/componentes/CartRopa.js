...
const CardsR = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(""); // Estado inicial vacío
  const router = useRouter();

  useEffect(() => {
    // Fetch de las categorías
    fetch("/categoriR.json")
      .then((response) => response.json())
      .then((data) => {
        setCategories(data);
        // Seleccionar automáticamente la primera categoría cargada
        if (data.length > 0) {
          setSelectedCategory(data[0].name); // Nombre de la primera categoría
        }
      })
  ...
  return (
    <div>
      <div className="header-section">
        <h1>Nuestros Productos de Cosméticos</h1>
